...
export function setupResults(dom) {
  ...
  async function pollForChatId(chatUrl, timeoutMs = 30000) {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      if (extractedId && extractedId.trim()) {
        ...
        console.log(
          `✅ [Results] Got chat_id after ${elapsed}ms:`,
          extractedId,
        );
        return extractedId;
      }
      ...
      if (Date.now() - start > 0 && (Date.now() - start) % 2000 < 500) {
        if (extractedId !== lastExtracted) {
          console.log(
            `⏳ [Results] Still polling... elapsed: ${Date.now() - start}ms, URL: ${chatUrl}`,
          );
    ...
    console.warn(
      `⏱️ [Results] Timeout polling for chat_id after ${timeoutMs}ms`,
    );
  ...
  function renderHistoryList(historyItems) {
    ...
    if (!historyItems || historyItems.length === 0) {
      historyList.innerHTML =
        '<p class="empty-state">Chưa có lịch sử. Chạy prompt để bắt đầu.</p>';
      return;
    }
  ...
  async function loadAndDisplayHistory(limit = 50) {
    try {
      ...
      if (historyList) {
        historyList.innerHTML =
          '<p class="empty-state" style="color: #d32f2f;">Lỗi tải lịch sử</p>';
      }
    }
  }
    ...
    try {
      ...
      if (response && response.type !== MESSAGE_TYPES.ERROR) {
        ...
        const extractedChatId =
          response.chatId || extractChatIdFromUrl(response.chatUrl);
        let chatIdToSave =
          extractedChatId && extractedChatId.trim() ? extractedChatId : null;
        ...
        if (response.chatUrl && chatIdToSave) {
          ...
          if (historyResponse?.errorCode) {
            console.error(
              "❌ [Results] Failed to save history:",
              historyResponse.errorMessage,
            );
          }
          ...
          console.warn(
            "❌ [Results] Could not get chat_id even after polling. Cannot save history.",
          );
          console.warn("[Results] Response object:", response);
          console.warn(
            "[Results] User can still manually refresh after ChatGPT responds",
          );
        }
      } else {
  ...
  function startPollingForResponse(chatId, historyId = null) {
    if (!chatId) {
      ...
      console.warn(
        "[Results] This usually means content script was not ready when prompt was sent",
      );
      return;
    }

    console.log("✅ [Results] Starting polling for response", {
      chatId,
      historyId,
    });
    ...
    currentPollInterval = setInterval(async () => {
      ...
      console.log(
        `[Results] 🔄 Poll ${pollCount}/${maxPolls} for chatId: ${chatId}`,
      );
      ...
      try {
        ...
        if (
          outputResponse &&
          outputResponse.type === MESSAGE_TYPES.CHATGPT_OUTPUT_READY
        ) {
          // Handler returns data.response (via createResponse spreading)
          const responseText =
            outputResponse.response || outputResponse.output || "";
          ...
          if (
            ...
            const updateData = {
              ...
              chat_url:
                outputResponse.chatUrl || `https://chatgpt.com/c/${chatId}`,
            };
            ...
            if (updateResponse.errorCode) {
              console.error(
                "❌ [Results] Failed to save response:",
            ...
            console.log(
              `⏳ [Results] Response not ready yet (length: ${responseText?.length || 0})`,
            );
          }
        } else if (outputResponse?.errorCode) {
          console.error(
            "[Results] ❌ Error getting output:",
