...
export class ClaudeWebProvider extends LLMProvider {
  ...
  async getStatus() {
    try {
      ...
      try {
        ...
        const response = await chrome.tabs.sendMessage(tabId, { action: 'ping' });
        if (response?.pong) {
          logger.info('getStatus: re-injection succeeded, Claude now connected', { tabId });
          return 'connected';
        }
      } catch (err) {
  ...
  async #ensureClaudeTab(forceNew = false) {
    ...
    if (!ready) {
      ...
      logger.info('ensureClaudeTab: ping failed, attempting content script re-injection', { tabId });
      try {
        await chrome.scripting.executeScript({
          target: { tabId },
          files: ['content-claude.js'],
        ...
        logger.warn('ensureClaudeTab: re-injection failed', { tabId, error: injectErr?.message });
      }
    }
