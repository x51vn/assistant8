...
describe('chatHistoryService', () => {
  ...
  describe('mergeMetadata()', () => {
    ...
    it('handles nested objects (shallow merge only)', () => {
      ...
      // Shallow merge - patch.capture replaces base.capture entirely
      expect(mergeMetadata(base, patch)).toEqual({
  ...
  describe('upsertOutboxItem()', () => {
    ...
    it('preserves existing items and adds new to front', () => {
      ...
      expect(result[0].runId).toBe('run-new'); // New at front
    ...
    it('merges data for existing runId (Phase 1 + Phase 2)', () => {
      ...
      expect(items[0].response).toBeNull(); // No response yet
      expect(items[0].metadata).toEqual({ source: 'SEND_PROMPT' });

      // Phase 2: Save response
      items = upsertOutboxItem(items, {
        runId: 'run-1',
      ...
      expect(items[0].prompt).toBe('What is AI?'); // Prompt preserved
      expect(items[0].response).toBe('AI is artificial intelligence'); // Response added
    ...
    it('preserves earliest timestamp across phases', () => {
      ...
      items = upsertOutboxItem(items, {
        runId: 'run-1',
        response: 'Response 1',
        timestamp: t2 // Later timestamp in Phase 2
      });

      expect(items[0].timestamp).toBe(t1); // Uses earliest
    });
    ...
    it('prefers non-empty patches for fields', () => {
      ...
      // Try to patch with empty/null values
      items = upsertOutboxItem(items, {
        runId: 'run-1',
        prompt: '', // Empty
        chatId: null // Null
      ...
      expect(items[0].prompt).toBe('Original prompt'); // Preserved
      expect(items[0].chatId).toBe('chat-1'); // Preserved
    });
  ...
  describe('Outbox quota management', () => {
    it('enforces OUTBOX_MAX_ITEMS=30 limit in saveOutbox', () => {
      ...
      // Simulating saveOutbox behavior
    ...
    it('supports aggressive shrink on quota exceeded', () => {
      ...
      const items = Array.from({ length: 35 }, (_, i) => ({
        runId: `run-${i}`,
        prompt: 'a'.repeat(50000), // Large text
        response: 'b'.repeat(100000)
      }));

      const safe = items.slice(0, OUTBOX_MAX_ITEMS);

      // Aggressive shrink to 5 items
  ...
  describe('Complex scenarios', () => {
    it('handles rapid Phase 1 + Phase 2 alternation', () => {
      ...
      // Simulate 3 concurrent prompts with their responses
      items = upsertOutboxItem(items, {
        runId: 'run-1',
        prompt: 'First prompt',
        metadata: { source: 'SEND_PROMPT' }
      ...
      expect(items[0].runId).toBe('run-3'); // Most recent at front
      expect(items[1].response).toBe('Second response'); // run-2 merged
      expect(items[2].response).toBe('First response'); // run-1 merged
    });
    ...
    it('preserves data integrity with max-char truncation', () => {
      ...
      items = upsertOutboxItem(items, {
        runId: 'run-1',
        prompt: 'a'.repeat(MAX_PROMPT + 1000) // Over limit
      });

      expect(items[0].prompt).toHaveLength(50_000 + 1000); // Not truncated yet (that happens on save)

      // Simulate Phase 2
      items = upsertOutboxItem(items, {
        runId: 'run-1',
        response: 'b'.repeat(MAX_RESPONSE + 5000) // Over limit
      });

      expect(items[0].response).toHaveLength(100_000 + 5000); // Not truncated yet

      // Truncation happens when normalizeText is called
    ...
    it('metadata merge from 3+ updates', () => {
      ...
      // All metadata merged (later overwrites earlier at same key)
      expect(items[0].metadata).toEqual({
        source: 'SEND_PROMPT',
        phase: 3,
