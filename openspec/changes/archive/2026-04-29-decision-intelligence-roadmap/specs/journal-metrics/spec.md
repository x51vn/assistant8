...
- `repeatedErrorRate` — tỷ lệ lỗi lặp lại theo `error_category` trong cửa sổ 30 ngày
- `disciplineScore` — điểm kỷ luật tổng hợp (0-100) dựa trên adherence, transition correctness, và guardrail history
- `insightAdoptionRate` — tỷ lệ áp dụng gợi ý playbook đã đánh dấu helpful
...
- **THEN** handler returns `{ totalTrades: 0, winCount: 0, lossCount: 0, winRate: null, avgRMultiple: null, ruleAdherenceRate: null, topErrors: [], periodTrades: 0, repeatedErrorRate: null, disciplineScore: null, insightAdoptionRate: null }`
...
The summary SHALL include: `openCount` (open entries), `plannedCount` (planned entries), `recentWinRate` (last 30 days), `avgRMultiple` (last 30 days), `disciplineScore` (last 30 days), and `topRepeatedErrors`.

#### Scenario: Fetch journal summary for Dashboard
- **WHEN** Dashboard requests journal summary
- **THEN** handler returns `openCount`, `plannedCount`, `recentWinRate`, `avgRMultiple`, `disciplineScore`, `topRepeatedErrors`
